  ...
  const enable_gps = () =>{
    if(Platform.OS === "android"){
      ...
      Geolocation.getCurrentPosition( async(position) => {
          navigation.navigate('Splash');
        }, error => alert(t('Please try again once')) , 
    ...
    <SafeAreaView style={styles.container}>
      <View style={{ height:'100%',width: '100%', justifyContent:'center'}}>
        ...
        <View style={{alignItems: 'center', justifyContent:'center', margin: 10}}>
          <Text style={{fontFamily:bold, fontSize:18, color:colors.theme_bg,}}>{t('Please allow')} {global.app_name} {t('to enable your GPS for accurate pickup')}.</Text>
        </View>
        <View style={{ margin:20}} /> 
        <TouchableOpacity onPress={enable_gps.bind(this)} style={styles.button}>
          <Text style={{ color:colors.theme_fg_three, fontFamily:bold}}>{t('Enable GPS')}</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>  
  )
