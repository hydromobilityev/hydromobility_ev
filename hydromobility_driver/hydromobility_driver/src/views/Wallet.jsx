      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
        ...
        <View style={{ paddingHorizontal: 24, marginTop: 16, marginBottom: 24 }}>
          <View style={{
            ...
            shadowColor: theme.shadow_color, shadowOffset: { width: 0, height: 12 },
            shadowOpacity: 0.08, shadowRadius: 16, elevation: Platform.OS == 'ios' ? 4 : 0, overflow: 'hidden'
          }}>
            <View style={{
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.6)', padding: 20 }}>
            <View style={{ width: '100%', maxWidth: 400, backgroundColor: theme.surface, borderRadius: 16, padding: 24, shadowColor: theme.shadow_color, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 20, elevation: 5 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
                ...
                <TouchableOpacity style={{ flex: 1, height: 48, borderRadius: 8, backgroundColor: theme.primary, justifyContent: 'center', alignItems: 'center', shadowColor: theme.primary, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, elevation: 2 }}
        ...
        <RBSheet
          ...
          <View style={{ margin: 10 }} />
          {payment_mode == 38 && <PaystackAndroid
            amount={amount}
            onSuccess={(res) => {
              console.log(res)

              call_add_wallet(); paystack_android_sheet.current.close(); setPaymentMode(0)
            }}
            onCancel={() => { Alert.alert('Payment Cancelled', 'You cancelled the payment'); paystack_android_sheet.current.close(); setPaymentMode(0) }}
            onError={(err) => { console.log(err); Alert.alert('Payment Error', 'Transaction failed'); paystack_android_sheet.current.close(); setPaymentMode(0) }}
          />}
        </RBSheet>
      </SafeAreaView>
    </PaystackProvider>
