...
import {
  api_url,
  corporate_customer_future_by_trip,
  GOOGLE_MAPS_API_KEY,
  ...
  const fetchRideDetailsFromAPI = async () => {
    ...
    try {
      const response = await fetch(
        `${api_url}${corporate_customer_future_by_trip}`,
        {
          method: "POST",
          headers: {
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-[70vh]">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate("/agent/future-rides")}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-1 gap-4 pt-4 border-t border-gray-200">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-xl font-bold text-gray-900">
                    {displayDistance || "N/A"} km
                    {routeDistance && !showDirectLine && (
                      <span className="text-sm text-gray-500 ml-2">
                        (via Google Maps)
                      </span>
                    )}
                    {showDirectLine && (
                      <span className="text-sm text-red-500 ml-2 block mt-1">
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                  <FaMapMarkerAlt /> {t("trip_route_map")}
                </h2>
                <p className="text-sm text-gray-600">
                  {(mapPickup?.address || "Pickup")?.split(",")[0]} to{" "}
                  {(mapDrop?.address || "Drop")?.split(",")[0]}
                </p>
                {isFetchingRoute && (
                ...
                {routeDistance && !isFetchingRoute && !showDirectLine && (
                  <div className="flex items-center gap-1 mt-1">
                    <span className="text-xs text-green-600">✓</span>
                    <span className="text-xs text-green-600">
                      Route calculated: {routeDistance} km (Google Maps)
                    </span>
                  </div>
                )}
                {showDirectLine && !isFetchingRoute && (
                  <div className="flex items-center gap-1 mt-1">
                    <span className="text-xs text-red-600">⚠</span>
                    <span className="text-xs text-red-600">
                      Showing straight line distance: {routeDistance} km
                    </span>
                  </div>
                )}
                {(!mapPickup?.lat || !mapDrop?.lat) && !isFetchingRoute && (
                  <p className="text-xs text-yellow-600 mt-1">
                    {!mapPickup?.lat && !mapDrop?.lat
                      ? "Both pickup and drop coordinates missing"
                      : !mapPickup?.lat
                        ? "Pickup coordinates missing"
                        : "Drop coordinates missing"}
                  </p>
                )}
              </div>
          ...
          {rideDetails.otp && (
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <FaShieldAlt /> {t("trip_otp")}
              </h2>
              <div className="text-center">
                <div className="inline-block px-8 py-6 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  <p className="text-4xl font-bold text-gray-900 tracking-widest">
                    {rideDetails.otp}
                  </p>
                </div>
                <p className="text-sm text-gray-500 mt-3">
                  {t("share_this_otp_with_the_driver_to_start_the_trip")}
                </p>
              </div>
            </div>
          )}
...
export default FutureRideDetails;
