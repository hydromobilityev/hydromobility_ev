...
import {
  api_url,
  corporate_customer_past_bookings,
...
const TodayRides = () => {
  ...
  const fetchTodayRides = async () => {
    try {
      ...
      const response = await fetch(
        `${api_url}${corporate_customer_past_bookings}`,
        {
          method: "POST",
          headers: {
  ...
  const handleTripClick = (tripId, rideData) => {
    ...
    navigate(`/past-ride-details/${tripId}`);
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="mt-4 text-gray-600">{t("loading_past_rides")}...</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {t("manage_all_rides_scheduled_for_past")}
            </p>
          </div>
