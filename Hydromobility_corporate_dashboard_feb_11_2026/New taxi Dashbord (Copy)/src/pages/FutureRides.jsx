...
import {
  api_url,
  corporate_customer_future_bookings,
...
const TodayRides = () => {
  ...
  const fetchTodayRides = async () => {
    try {
      ...
      const response = await fetch(
        `${api_url}${corporate_customer_future_bookings}`,
        {
          method: "POST",
          headers: {
  ...
  const cancelTrip = async (tripId) => {
    try {
      ...
      if (response.data.status === 1) {
        ...
        toast.error(response.data.message || t("failed_to_cancel_trip"));
      }
    } catch (error) {
      toast.error(
  ...
  const handleTripClick = (tripId, rideData) => {
    ...
    navigate(`/future-ride-details/${tripId}`);
  };
...
`;
  ...
  const handleCancelRide = async (rideId, tripId, rideStatus, e) => {
    ...
    if (![1, 2, 3].includes(rideStatus)) {
      alert(`Cannot cancel ride. Current status: ${rideStatus}`);
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="mt-4 text-gray-600">{t("loading_future_rides")}...</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {t("manage_all_rides_scheduled_for_future")}
            </p>
          </div>
