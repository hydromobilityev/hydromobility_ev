...
class CustomerController extends Controller
{
    ...
    public function get_wallet(Request $request)
    {
        ...
        if ($input['lang'] == 'en') {
            ...
            $data['all'] = CustomerWalletHistory::select('id', 'type', 'message_ar as message', 'amount', 'created_at', 'updated_at')->where('customer_id', $input['id'])->orderBy('id', 'desc')->get();
            $data['expenses'] = CustomerWalletHistory::select('id', 'type', 'message_ar as message', 'amount', 'created_at', 'updated_at')->where('customer_id', $input['id'])->where('type', 2)->orderBy('id', 'desc')->get();
            $data['receives'] = CustomerWalletHistory::select('id', 'type', 'message_ar as message', 'amount', 'created_at', 'updated_at')->where('customer_id', $input['id'])->where('type', 1)->orderBy('id', 'desc')->get();
        }
    ...
    public function register(Request $request)
    {
        ...
        if (is_object($customer)) {
            ...
            if (env('MODE') == 'DEMO') {
                ...
                $this->lead_inject($input['first_name'], $input['phone_with_code'], $input['email']);
            }
            return response()->json([
                "result" => $customer,
                "message" => 'Registered Successfully',
    ...
    public function delete_customer(Request $request)
    {
        ...
        $database->getReference('customers/' . $customer->id)->remove();

        CustomerWalletHistory::where('customer_id', $customer->id)->delete();
    ...
    public function get_recent_searches(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'customer_id' => 'required',
        ...
        $data = DB::table('trip_requests')->where('customer_id', $input['customer_id'])->select('drop_address as address', 'drop_lat as lat', 'drop_lng as lng')->orderBy('id', 'desc')->take(5)->get();
        return response()->json([
            "result" => $data,
            "message" => 'Success',
            "status" => 1
    ...
    public function lead_inject($customer_name, $customer_phone, $customer_email)
    {
        // set post fields
        $post = [
            'source_id' => 7,
        ...
        $ch = curl_init('https://menpanitech.com/api/update_lead');
    ...
    public function delete_account(Request $request)
    {
        ...
        $factory = (new Factory())->withDatabaseUri(env('FIREBASE_DB'));
        $database = $factory->createDatabase();
        $delete_path = 'customers/' . $input['customer_id'];
        if (Customer::where('phone_with_code', $input['phone_with_code'])->where('id', $input['customer_id'])->update(['is_deleted' => 1])) {
    ...
    public function m_pesa($customer_id, $amount)
    {
        ...
        $payload = [
            ...
            "CallBackURL"       => url("/api/m_pesa/callback/{$customer_id}"),
        ...
        if (curl_errno($curl)) {
            Log::error('MPesa STK Push Error', ['error' => curl_error($curl)]);
        }
    ...
    public function callback(Request $request, $customer_id)
    {
        ...
        if ($resultCode === 0) {
            $this->send_mpesa_fcm($customer_id, 1, $fcm_token);
        } else {
            $this->send_mpesa_fcm($customer_id, 0, $fcm_token);
        }

        return response()->json(['ResultCode' => 0, 'ResultDesc' => 'Accepted']);
    }

    public function check_status($checkoutRequestId)
    {
        $token = $this->get_access_token();
