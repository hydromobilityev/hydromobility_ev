...
class BookingController extends Controller
{
    public function find_driver($trip_request_id)
    {
        ...
        if ($trip_request->trip_type != 5) {
            $gth_drivers = $this->find_gth_drivers($trip_request->drop_lat, $trip_request->drop_lng);
            foreach ($drivers as $key => $value) {
                if ($value && array_key_exists('driver_id', $value)) {
                    if (!in_array($value['driver_id'], $rejected_drivers)) {
                        ...
                        $dr = Driver::where('id', $value['driver_id'])->first();
                        $ride_settings = $this->check_driver_trip_settings($trip_request->trip_type, $value['driver_id']);
                        if ($value['driver_id'] == 1) {
                            //echo $booking_searching_radius.'-'.$value['online_status'].'-'.$value['booking']['booking_status'].'-'.$dr->wallet;exit;
                        }
                        if ($distance <= $booking_searching_radius && $value['online_status'] == 1 && $value['booking']['booking_status'] == 0 && $dr->wallet > -500 && $ride_settings == 1 && $dr->fcm_token != '' && $dr->fcm_token != '123456') {
                            if (in_array($value['driver_id'], $gth_drivers)) {
                                $gth_driver_id = $value['driver_id'];
        ...
        $this->send_offline_booking_fcm($trip_request->id, $min_driver_id);
    ...
    public function get_driver_today_bookings(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'driver_id' => 'required',
        ...
        $data = DB::table('trips')
            ...
            ->whereDate('trips.pickup_date', date('Y-m-d'))
            ->orderBy('id', 'DESC')
            ->get();
        return response()->json([
            "result" => $data,
        ...
        if ($input['lang'] == 'en') {
            if ($input['filter'] == 1) {
                $data = DB::table('trips')
                    ...
                    ->select('trips.*', 'customers.first_name as customer_name', 'drivers.first_name as driver_name', 'customers.profile_picture', 'payment_methods.payment', 'driver_vehicles.brand', 'driver_vehicles.color', 'driver_vehicles.vehicle_name', 'driver_vehicles.vehicle_number', 'trip_types.name as trip_type', 'booking_statuses.status_name', 'vehicle_categories.vehicle_type', 'cancellation_reasons.reason')
                    ->where('trips.driver_id', $input['driver_id'])
                    ->whereIn('trips.status', [1, 2, 3, 4, 5, 6, 7])->orderBy('id', 'DESC')
                    ->get();
                ...
                $data = DB::table('trips')
                    ...
                    ->select('trips.*', 'customers.first_name as customer_name', 'drivers.first_name as driver_name', 'customers.profile_picture', 'payment_methods.payment', 'driver_vehicles.brand', 'driver_vehicles.color', 'driver_vehicles.vehicle_name', 'driver_vehicles.vehicle_number', 'trip_types.name as trip_type', 'booking_statuses.status_name', 'vehicle_categories.vehicle_type')
                    ->where('trips.driver_id', $input['driver_id'])
                    ->whereIn('trips.status', [5])->orderBy('id', 'DESC')
                    ->get();
            }
        } else {
            if ($input['filter'] == 1) {
    ...
    public function calculate_earnings($trip_id)
    {
        ...
        Driver::where('id', $trip->driver_id)->update(['wallet' => $new_wallet, 'gth_status' => 0]);
    }
    ...
    public function get_estimation_fare(Request $request)
    {
        ...
        if ($input['lang'] == 'en') {
            $vehicles = VehicleCategory::select('id', 'vehicle_type', 'active_icon', 'inactive_icon', 'description', 'status')->where('vehicle_mode', $vehicle_mode)->where('status', 1)->get();
        } else {
            $vehicles = VehicleCategory::select('id', 'vehicle_type_ar as vehicle_type', 'base_fare', 'price_per_km', 'active_icon', 'inactive_icon', 'description_ar as description', 'status')->where('vehicle_mode', $vehicle_mode)->where('status', 1)->get();
        }
        ...
        foreach ($vehicles as $key => $value) {
            ...
            //$vehicles[$key]->eta = $this->find_nearest_driver_eta($input['pickup_lat'],$input['pickup_lng'],$value->id,$input['trip_type']);
            $vehicles[$key]->eta = 'Nearest';
        }
    ...
    public function get_driving_distance($lat1, $lat2, $long1, $long2)
    {
        ...
        // Use custom routing (GraphHopper/Routemappy)
        if ($map->routing == 1) {
            $url = "https://api.routemappy.com/route?from={$lat1},{$long1}&to={$lat2},{$long2}&key=" . env('RMP_MAP_KEY');
            ...
            if (isset($json['paths'][0])) {
                $meters = $json['paths'][0]['distance']; // meters
                $seconds = $json['paths'][0]['time'] / 1000; // milliseconds to seconds

                $km = round($meters / 1000, 2);
                $min = round($seconds / 60);

                $data['distance_with_mes'] = $km . ' km';
                $data['distance'] = $km;
                $data['time'] = $min . ' mins';
            } else {
                $data['distance_with_mes'] = '0 km';
                $data['distance'] = 0;
                $data['time'] = '0 mins';
            }
        } else {
    ...
    {
        $data = [];
        $package_price = DB::table('rental_fare_management')->where('vehicle_type', $vehicle_type)->where('package_id', $package_id)->first();
        ...
        if ($input['lang'] == 'en') {
            if ($input['filter'] == 1) {
                $data = DB::table('trips')
                    ...
                    ->leftJoin('trip_cancellations', 'trip_cancellations.trip_id', 'trips.id')
                    ->leftJoin('cancellation_reasons', 'cancellation_reasons.id', 'trip_cancellations.reason_id')
                    ->select('trips.*', 'customers.first_name as customer_name', 'drivers.first_name as driver_name', 'drivers.profile_picture', 'payment_methods.payment', 'driver_vehicles.brand', 'driver_vehicles.color', 'driver_vehicles.vehicle_name', 'driver_vehicles.vehicle_number', 'trip_types.name as trip_type', 'booking_statuses.status_name', 'vehicle_categories.vehicle_type', 'cancellation_reasons.reason')
                    ->where('trips.customer_id', $input['customer_id'])
                    ->whereIn('trips.status', [1, 2, 3, 4, 5, 6, 7])
                    ->orderBy('id', 'DESC')
                    ->get();
            } else if ($input['filter'] == 2) {
                $data = DB::table('trips')
    ...
    public function find_gth_drivers($gth_lat, $gth_lng)
    {
        $gth_drivers = Driver::where('gth_status', 1)->get();
        $radius = TripSetting::value('gth_searching_radius');
        $data = [];
        foreach ($gth_drivers as $key => $value) {
            $distance = $this->distance($gth_lat, $gth_lng, $value->gth_lat, $value->gth_lng, 'K');
            if ($distance <= $radius) {
    ...
    public function get_eta($pickup_lat, $pickup_lng, $drop_lat, $drop_lng)
    {
        ...
        if (@$result->routes[0]->legs[0]->duration->text) {
            $duration = str_replace(" km", "", $result->routes[0]->legs[0]->duration->text);
            return $duration;
        } else {
            return 0;
        }
    ...
    public function find_nearest_driver_eta($lat, $lng, $vehicle_type, $trip_type)
    {
        $factory = (new Factory())->withDatabaseUri(env('FIREBASE_DB'));
        $database = $factory->createDatabase();
        $drivers = $database->getReference('drivers/' . $vehicle_type)
            ->getSnapshot()->getValue();
        $booking_searching_radius = TripSetting::value('booking_searching_radius');
        if (!is_array($drivers)) {
            return 0;
        ...
        foreach ($drivers as $key => $value) {
            if ($value && array_key_exists('driver_id', $value)) {
                $distance = $this->distance($lat, $lng, $value['geo']['lat'], $value['geo']['lng'], 'K');
                $dr = Driver::where('id', $value['driver_id'])->first();
                $ride_settings = $this->check_driver_trip_settings($trip_type, $value['driver_id']);
                /*if($vehicle_type == 2){
                    echo 'Distance : '.$distance.'-'.'Online status : '.$value['online_status'].'-'.'Booking Status : '.$value['booking']['booking_status'].'-'.'Wallet :'.$dr->wallet.'-'.' Ride setting : '.$ride_settings;
                    echo '</br>';
                    echo $lat.'-'.$lng.'-'.$value['geo']['lat'].'-'.$value['geo']['lng'];
                }*/
                if ($distance <= $booking_searching_radius && $value['online_status'] == 1 && $value['booking']['booking_status'] == 0 && $dr->wallet > -500 && $ride_settings == 1) {
                    if ($dr->gth_status == 0) {
                        $earning = $this->get_today_driver_earnings($value['driver_id']);
                        if ($min_earning == 0 && $i == 0) {
                            $min_earning = $earning;
        ...
        if ($n_lat != 0 && $n_lng != 0) {
            $eta = $this->get_eta($lat, $lng, $n_lat, $n_lng);
            return $eta;
        } else {
            return 0;
        }
    ...
    {
        $input = $request->all();
        /*  if (!empty($input['corporate_customer_id']) && empty($input['customer_id'])) {
            $input['customer_id'] = 0;
            $input['customer_name'] = $input['customer_name'] ?? null;
        } */
        if (!empty($input['corporate_customer_id']) && empty($input['customer_id'])) {
            $input['customer_id'] = 0;
            $input['agent_name'] = $input['agent_name'] ?? null;
            $input['agent_phone_number'] = $input['agent_phone_number'] ?? null;
        }
        ...
        if ($input['trip_type'] == 3) {
            $from_zone = $input['zone'];
            $to_zone = $this->get_zone($input['drop_lat'], $input['drop_lng']);
            if ($from_zone == $to_zone) {
                return response()->json([
                    "message" => 'Destination city cannot be same as pickup city for outstation trips.',
                    "status" => 0
                ]);
            }
        }
        ...
        if ($input['trip_type'] == 1) {
            $from_zone = $input['zone'];
            $to_zone = $this->get_zone($input['drop_lat'], $input['drop_lng']);
            //echo $from_zone.'-'.$to_zone;exit;
            if ($from_zone != $to_zone) {
                return response()->json([
                    "message" => 'Please book with outstation mode.',
                    "status" => 0
                ]);
            }
        }
        ...
        if ($input['trip_type'] != 5) {
            $gth_drivers = $this->find_gth_drivers($input['drop_lat'], $input['drop_lng']);
            foreach ($drivers as $key => $value) {
                if ($value && array_key_exists('driver_id', $value)) {
                    $distance = $this->distance($input['pickup_lat'], $input['pickup_lng'], $value['geo']['lat'], $value['geo']['lng'], 'K');
                    $dr = Driver::where('id', $value['driver_id'])->first();
                    $ride_settings = $this->check_driver_trip_settings($input['trip_type'], $value['driver_id']);
                    /*if($value['driver_id'] == 3){
                        echo $distance.'-'.$booking_searching_radius.'-'.$value['online_status'].'-'.$value['booking']['booking_status'].'-'.$dr->wallet.'-'.$ride_settings.'-'.$dr->fcm_token;
                        exit;
                    }
                    */
                    if ($distance <= $booking_searching_radius && $value['online_status'] == 1 && $value['booking']['booking_status'] == 0 && $dr->wallet > -500 && $ride_settings == 1 && $dr->fcm_token != '' && $dr->fcm_token != '123456') {
        ...
        //echo $min_driver_id.'-'.$input['booking_type'];exit;
        ...
        if (!empty($input['corporate_customer_id'])) {
            $agent = CorporateAgent::find($input['corporate_customer_id']);
            if ($agent) {
                $corporate_customer_id = $agent->corporate_customer_id;

                $company_name = CorporateCustomer::where('id', $corporate_customer_id)
                    ->value('company_name');
            }
        }
        ...
        unset($booking_request['km']);
        $booking_request['total'] = $fares['total_fare'] ?? 0;
        $booking_request['sub_total'] = $fares['fare'] ?? 0;
        $booking_request['discount'] = $fares['discount'];
        $booking_request['tax'] = $fares['tax'] ?? 0;
        $booking_request['static_map'] = $img;
        $booking_request['driver_id'] = $min_driver_id;
        $booking_request['corporate_customer_id'] = $input['corporate_customer_id'] ?? null;
        $booking_request['agent_name'] = $input['agent_name'] ?? null;
        $booking_request['agent_phone_number'] = $input['agent_phone_number'] ?? null;
        $booking_request['company_name'] = $company_name;

        $id = TripRequest::create($booking_request)->id;


        if (!empty($input['agent_phone_number'])) {
            $pickupDate = date('F j, Y \a\t H:i', strtotime($input['pickup_date']));
            $pickupAddress = $input['pickup_address'];
            $message = "Hydromobility EV Ride Scheduled 🚗\n\n"
                . "Date: {$pickupDate}\n"
                . "Pickup: {$pickupAddress}";
            $phone = $input['agent_phone_number'];
        ...
        $this->send_offline_booking_fcm($id, $min_driver_id);
        if ($stops) {
        ...
        if ($input['trip_type'] == 5 && $shared_ride_mode == 2) {
            $newPost = $database
                ...
                ->update([
                    ...
                    'corporate_customer_id' => $input['corporate_customer_id'] ?? null,
                    'agent_name' => $input['agent_name'] ?? null,
                    'agent_phone_number' => $input['agent_phone_number'] ?? null,
    ...
    public function send_offline_booking_fcm($trip_id, $driver_id)
    {
        $fcm_token = Driver::where('id', $driver_id)->value('fcm_token');
        if ($fcm_token) {
            $this->send_booking_fcm($trip_id, $fcm_token, $driver_id);
        }
    }

    public function check_driver_trip_settings($trip_type, $driver_id)
    {
        $dr = Driver::where('id', $driver_id)->first();
        return 1;
        if ($trip_type == 1) {
            return $dr->daily_ride_status;
        } else if ($trip_type == 2) {
            return $dr->rental_ride_status;
        } else if ($trip_type == 3) {
            return $dr->outstation_ride_status;
        } else if ($trip_type == 4) {
            return 1;
        } else if ($trip_type == 5) {
            return $dr->shared_ride_status;
        }
    }
    ...
    public function trip_accept(Request $request)
    {
        ...
        if ($trip['booking_type'] == 1 && $trip['status'] != 1 && $trip['status'] != 4) {
            return response()->json([
                "message" => 'Sorry booking not available',
                "status" => 0
            ]);
        ...
        $phone_with_code = null;
        $customer = null;

        if ($customer_id != 0) {
            $customer = Customer::where('id', $customer_id)->first();
            $phone_with_code = $customer ? $customer->phone_with_code : null;
        }
        $data = $trip;
        $data['driver_id'] = $input['driver_id'];
        $data['promo_code'] = $data['promo'];
        ...
        $agent_phone_number = TripRequest::where('id', $input['trip_id'])
            ->value('agent_phone_number');

        if (!empty($agent_phone_number)) {
            $phone = str_starts_with($agent_phone_number, '+')
                ? $agent_phone_number
                : '+' . $agent_phone_number;

            $otpMessage = "Hydromobility OTP\n\nYour trip OTP is {$otp}";
        ...
        if ($customer && $customer->fcm_token != "" && $customer->fcm_token != "123456") {
            $this->send_fcm($current_status->status_name, $current_status->customer_status_name, $customer->fcm_token);
        }
    ...
    public function trip_reject(Request $request)
    {
        ...
        DriverTripRequest::create($data);
        if ($trip->booking_type == 1 && in_array($trip->status, [1, 4])) {
            TripRequest::where('id', $input['trip_id'])->update(['status' => 4, 'flag' => 0]);
    ...
    public function trip_request_details(Request $request)
    {
        ...
        TripRequest::where('id', $input['trip_request_id'])->update(['flag' => 1]);
        $data = DB::table('trip_requests')
            ...
            ->select('trip_requests.*',  'trip_requests.corporate_customer_id', 'customers.first_name', 'trip_types.name as trip_type_name')
            ->first();

        if ($data && $data->corporate_customer_id && $data->corporate_customer_id != 0) {
            $corporate = DB::table('corporate_agents')
                ->where('id', $data->corporate_customer_id)
                ->select('first_name')
                ->first();

            if ($corporate) {
                $data->first_name = $corporate->first_name;
            }
        }
    ...
    public function change_trip_status(Request $request)
    {
        ...
        if ($input['status'] == 4) {
            ...
            if ($trip->customer_id > 0) {
                $database->getReference('customers/' . $trip->customer_id)
                    ->update([
                        'booking_id' => 0,
                        'booking_status' => 0,
                        'is_searching' => 0
    ...
    public function get_distance($trip_id)
    {
        ...
        /*$factory = (new Factory())->withDatabaseUri(env('FIREBASE_DB'));
        $database = $factory->createDatabase();
        $waypoints = $database->getReference('trips/' . $trip_id . '/waypoints')
            ->getSnapshot()->getValue();
        if (!$waypoints) {
            $waypoints = [];
        } else {
        ...
        $trip = Trip::where('id', $trip_id)->first();
        $data = $this->getMatchedRouteDistanceAndPolyline($trip->actual_pickup_lat . ',' . $trip->actual_pickup_lng, $trip->actual_drop_lat . ',' . $trip->actual_drop_lng, $waypoints);

        Trip::where('id', $trip_id)->update(['distance' => $data['distance_kilometers'], 'polyline' => $data['latest_polyline']]);

        return $data['distance_kilometers'];*/
        // Google API fallback
        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins={$trip->actual_pickup_lat},{$trip->actual_pickup_lng}&destinations={$trip->actual_drop_lat},{$trip->actual_drop_lng}&mode=driving&language=pl-PL&key=" . env('MAP_KEY');
        //echo $url;exit;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    public function trip_cancel_by_customer(Request $request)
    {
        ...
        $this->send_cancel_booking_fcm($trip_id, $fcm_token, $driver_id);
        return response()->json([
            "message" => 'Success',
            "status" => 1
        ]);
    ...
    public function trip_cancel_by_corporate(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
        ...
        $database->getReference('customers/' . $trip->customer_id)->update([
            'booking_id'     => 0,
            'booking_status' => 0,
            'is_searching'   => 0
        ]);

        $vehicle_type = DriverVehicle::where('id', $trip->vehicle_id)
            ->value('vehicle_type');

        $database->getReference(
            'drivers/' . $vehicle_type . '/' . $trip->driver_id . '/booking'
        )->update([
            'booking_id'     => 0,
            'booking_status' => 0,
        ]);

        $database->getReference('trips/' . $trip->id)->update([
            'status' => $cancelStatus
        ]);

        return response()->json([
            'status'  => 1,
            'message' => 'Trip cancelled by corporate successfully'
        ]);
    }
    ...
    public function get_bill(Request $request)
    {
        ...
        $trip->payment_mode = DB::table('payment_methods')->where('id', $trip->payment_method)->value('payment');
        if ($trip->status == 4 || $trip->status == 5) {
            $trip->driver_commission = DB::table('driver_earnings')->where('trip_id', $trip->id)->value('amount');
            $ac = ($trip->total - $trip->driver_commission);
            $trip->admin_commission = number_format((float)$ac, 2, '.', '');
        } else {
            $trip->driver_commission = 0;
            $trip->admin_commission = 0;
        }
        return response()->json([
            "result" => $trip,
            "message" => 'Success',
    ...
    public function getMatchedRouteDistanceAndPolyline($from, $to, array $waypoints = [])
    {
        $segments = [];
        $allCoordinates = [];

        // Step 1: Convert 'from' and 'to'
        [$fromLat, $fromLng] = explode(',', $from);
        [$toLat, $toLng] = explode(',', $to);

        // Default timestamps
        $defaultStart = time();
        $defaultEnd = $defaultStart + 60;

        // If waypoints provided, convert them and determine time range
        if (!empty($waypoints)) {
            $convertedWaypoints = array_map(function ($point) {
                return [
                    'lat' => (float) $point['lat'],
                    'lon' => (float) $point['lng'],
            ...
            usort($convertedWaypoints, fn($a, $b) => $a['time'] <=> $b['time']);
            $firstTime = $convertedWaypoints[0]['time'];
            $lastTime = $convertedWaypoints[count($convertedWaypoints) - 1]['time'];
        } else {
            // No waypoints: set default time range
            $convertedWaypoints = [];
            $firstTime = $defaultStart;
            $lastTime = $defaultEnd;
        }
        ...
        // Merge all points
        $allPoints = array_merge([$fromPoint], $convertedWaypoints, [$toPoint]);

        // Step 2: Chunk into max 25 points and call API
        $maxChunk = 25;
        $chunks = array_chunk($allPoints, $maxChunk);

        foreach ($chunks as $chunk) {
            $response = Http::get('https://api.routemappy.com/mapmatch', [
                'key' => env('RMP_MAP_KEY'),
                'points' => json_encode($chunk)
            ]);

            if (!$response->ok()) {
                return [
                    'error' => 'Map Match API failed',
            ...
            $data = $response->json();
            $path = $data['paths'][0] ?? null;

            if ($path) {
                $segments[] = $path['distance'];
                $decoded = $this->decodePolyline($path['points']);
                $allCoordinates = array_merge($allCoordinates, $decoded);
            }
        }

        // Step 3: Final distance + polyline result
        $totalMeters = array_sum($segments);
        $totalKilometers = round($totalMeters / 1000, 2);
        $totalMiles = round($totalMeters / 1609.344, 2);

        return [
            'distance_meters' => $totalMeters,
            'distance_kilometers' => $totalKilometers,
            'distance_miles' => $totalMiles,
            'latest_polyline' => $this->encodePolyline($allCoordinates),
        ];
    }


    public function simplifyWaypoints(array $points, float $tolerance = 0.0005): array
    {
        ...
        if ($maxDistance > $tolerance) {
            $firstHalf = array_slice($points, 0, $index + 1);
            $secondHalf = array_slice($points, $index);

            $result1 = $this->simplifyWaypoints($firstHalf, $tolerance);
            $result2 = $this->simplifyWaypoints($secondHalf, $tolerance);

            array_pop($result1); // avoid duplicate
    ...
    {
        if ($start == $end) {
            return sqrt(pow($point['lat'] - $start['lat'], 2) + pow($point['lng'] - $start['lng'], 2));
        }
    ...
    public function reduceWaypoints($waypoints)
    {
        $input['waypoints'] = $waypoints;
        $input['tolerance'] = 0.0005;
        $simplified = $this->simplifyWaypoints($input['waypoints'], $input['tolerance']);

        $data['original_count'] = count($waypoints);
        $data['simplified_count'] = count($simplified);
