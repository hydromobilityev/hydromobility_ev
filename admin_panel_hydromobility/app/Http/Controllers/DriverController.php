...
class DriverController extends Controller
{
    ...
    public function profile_update(Request $request)
     ...
     public function driver_earnings(Request $request){
        ...
        $month_earnings = DriverEarning::where('driver_id',$input['id'])->whereYear('created_at', Carbon::now()->year)->whereMonth('created_at', Carbon::now()->month)->sum("amount");
        $week_earnings = DriverEarning::where('driver_id',$input['id'])->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->sum("amount");
    ...
    public function get_driver_settings(Request $request){
        ...
        $data = DB::table('drivers')->select('daily_ride_status','rental_ride_status','outstation_ride_status','shared_ride_status')->where('id','=',$input['driver_id'])->first();
        return response()->json([
            "data" =>$data,
            "message" => 'Success',
            "status" => 1
    ...
    public function change_driver_settings(Request $request){
        ...
        $validator = Validator::make($input, [
            ...
            'daily_ride_status' => 'required',
            'rental_ride_status' => 'required',
            'outstation_ride_status' => 'required',
        ]);
        if ($validator->fails()) {
            return $this->sendError($validator->errors());
        }
    ...
    public function driver_withdrawal_request(Request $request){
        ...
        $input['existing_wallet'] = $old_wallet;
    ...
    public function app_settings()
    {
        ...
        $data->map_settings = DB::table('map_settings')->first();
        $data->mode = env('MODE');
        $data->rmp_map_key = env('RMP_MAP_KEY');
        return response()->json([
            "result" => $data,
            "message" => 'Success',
            "status" => 1
    ...
    public function check_document_status(Request $request){
        ...
        $vehicle_status = DriverVehicle::where('driver_id',$input['driver_id'])->first();
                
        $vehicle_document_status = DB::table('drivers')->join('driver_vehicles','driver_vehicles.driver_id','=','drivers.id')->where('drivers.id',$input['driver_id'])->where('driver_vehicles.vehicle_certificate_status',16)->where('driver_vehicles.vehicle_insurance_status',16)->where('driver_vehicles.vehicle_image_status',16)->where('drivers.id_proof_status',16)->count();
        ...
        if (Hash::check($credentials['password'], $driver->password)) {
            if($driver->status == 1){
                ...
                if($driver->id_proof_status && is_object($vehicle_status) && $vehicle_document_status){
                    ...
                    $driver->vehicle_mode = DB::table('vehicle_categories')->where('id',$vehicle_status->vehicle_type)->value('vehicle_mode');
                    $driver->approved_status = 1;
                    return response()->json([
                        "result" => $driver,
                        "message" => 'Success',
                        "status" => 1
                    ...
                    if(!is_object($vehicle_status)){
                        $driver->approved_status = 2;
                    }else{
                        $document_upload_status = DB::table('drivers')->join('driver_vehicles','driver_vehicles.driver_id','=','drivers.id')->where('drivers.id',$driver->id)->where('driver_vehicles.vehicle_certificate_status',15)->where('driver_vehicles.vehicle_insurance_status',15)->where('driver_vehicles.vehicle_image_status',15)->where('drivers.id_proof_status',15)->count();
                        if($document_upload_status){
                            $driver->approved_status = 3;
                        }else{
                            $driver->approved_status = 4;
                        }
                    }
                    $driver->vehicle_type = 0;
                    $driver->vehicle_mode = 0;
                    return response()->json([
                        "result" => $driver,
                        "message" => 'Success',
                        "status" => 1
    ...
    public function change_online_status(Request $request){
        ...
        if($driver->id_proof_status && is_object($vehicle_status) && $vehicle_document_status){
            ...
            if($input['type'] == 1){
                Driver::where('id',$input['id'])->update([ 'gth_status' => 0 ]);   
            }
            return response()->json([
                "message" => 'Success',
                "status" => 1
    ...
    public function driver_dashboard(Request $request){
        ...
        $result['request_booking_id'] = TripRequest::where('driver_id',$input['id'])->where('status',1)->value('id');
        if(!$result['request_booking_id']){
            $result['request_booking_id'] = 0;
        }
        ...
        $result['gth_status'] = $driver->gth_status;
        if($result){
            return response()->json([
                "result" => $result,
                "message" => 'Success',
    ...
    public function gth_location_change(Request $request){
        $input = $request->all();
        $validator = Validator::make($input, [
            'driver_id' => 'required',
            'gth_location' => 'required',
            'gth_lat' => 'required',
            'gth_lng' => 'required'
        ]);
        if ($validator->fails()) {
        ...
        $vehicle_document_status = DB::table('drivers')->join('driver_vehicles','driver_vehicles.driver_id','=','drivers.id')->where('drivers.id',$driver->id)->where('driver_vehicles.vehicle_certificate_status',16)->where('driver_vehicles.vehicle_insurance_status',16)->where('driver_vehicles.vehicle_image_status',16)->where('drivers.id_proof_status',16)->count();
        if($driver->id_proof_status && is_object($vehicle_status) && $vehicle_document_status){
            Driver::where('id',$input['driver_id'])->update([ 'online_status' => 1]);
            $driver = Driver::where('id',$input['driver_id'])->first();
            ...
            if(Driver::where('id',$input['driver_id'])->update([ 'gth_location' => $input['gth_location'], 'gth_lat' => $input['gth_lat'], 'gth_lng' => $input['gth_lng'], 'gth_status' => 1 ])){
                return response()->json([
                    "message" => 'Success',
                    "status" => 1
                ]);
    ...
    public function gth_get_location(Request $request){
        $input = $request->all();
        $validator = Validator::make($input, [
            'driver_id' => 'required',
        ]);
        ...
        $data = Driver::where('id',$input['driver_id'])->select('gth_location','gth_lat','gth_lng')->first();
        if($data){
            return response()->json([
                "result" => $data,
                "message" => 'Success',
    ...
    public function delete_account(Request $request){
        ...
        $factory = (new Factory())->withDatabaseUri(env('FIREBASE_DB'));
        $database = $factory->createDatabase();
        $vehicle_type = DB::table('driver_vehicles')->where('driver_id',$input['driver_id'])->value('vehicle_type');
        $delete_path = '/drivers/'.$vehicle_type.'/'.$input['driver_id'];
        if(Driver::where('phone_with_code',$input['phone_with_code'])->where('id',$input['driver_id'])->update([ 'is_deleted' => 1 ])){
            $database->getReference($delete_path)->remove();
            return response()->json([
                "message" => 'Success',
                "status" => 1
            ]);
