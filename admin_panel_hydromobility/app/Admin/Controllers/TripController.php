...
class TripController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('corporate_customer_id', __('Ride Booker'))->display(function ($corporate_agent) {
            return CorporateAgent::where('id', $corporate_agent)->value('first_name') ?: '-';
        });
        $grid->column('agent_name', __('Ride Booker Name'))->display(function () {
            return $this->agent_name ?? '-';
        });
        $grid->column('agent_phone_number', __('Ride Booker Number'))->display(function () {
            return $this->agent_phone_number ?? '-';
        });
        $grid->column('driver_id', __('Driver name'))->display(function ($driver) {
            $driver_name = Driver::where('id', $driver)->value('first_name');
            return "$driver_name";
        });
        $grid->column('company_name', __('Company'))->display(function () {
            return $this->company_name ?? '-';
        });
        $grid->column('customer_note', __('Customer Note'))->display(function () {
            return $this->customer_note ?? '-';
        });
        $grid->column('driver_note', __('Driver Note'))->display(function () {
        ...
        $grid->column('cancellation_reason', __('Cancellation Reason'))->display(function ($cancellation_reason) {
            if ($this->status > 5) {
                $reason_id = DB::table('trip_cancellations')->where('trip_id', $this->id)->value('reason_id');
                return DB::table('cancellation_reasons')->where('id', $reason_id)->value('reason');
            } else {
                return '---';
            }
        });
        ...
        $grid->filter(function ($filter) {
            ...
            $corporate_customer = CorporateAgent::pluck('first_name', 'id');
            ...
            $filter->equal('corporate_customer_id', 'Ride Booker')->select($corporate_customer);
    ...
    protected function form()
    {
        ...
        $form->textarea('actual_pickup_address', __('Actual Pickup Address'))->rules(function ($form) {
            return 'required';
        });
        $form->textarea('actual_drop_address', __('Actual Drop Address'))->rules(function ($form) {
            return 'required';
        });
        ...
        $form->textarea('actual_pickup_lat', __('Actual Pickup latitude'))->rules(function ($form) {
            return 'required';
        });
        $form->textarea('actual_pickup_lng', __('Actual Pickup logitude'))->rules(function ($form) {
            return 'required';
        });
        $form->textarea('actual_drop_lat', __('Actual Drop latitude'))->rules(function ($form) {
            return 'required';
        });
        $form->textarea('actual_drop_lng', __('Actual Drop logitude'))->rules(function ($form) {
            return 'required';
        });
