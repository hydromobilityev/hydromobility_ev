...
class CreateTripController extends AdminController
{
    ...
    protected function form()
    {
        ...
        $capital = DB::table('app_settings')->first();
        $map_settings = DB::table('map_settings')->first();
        $defaultLat = $capital->capital_lat;
        $defaultLng = $capital->capital_lng;
        $geoCodeType = $map_settings->places;
        ...
        $form->text('pickup_address', 'Pickup Address')->attribute(['id' => 'pickup_autocomplete']);
        $form->hidden('pickup_lat')->attribute(['id' => 'pickup_lat']);
        $form->hidden('pickup_lng')->attribute(['id' => 'pickup_lng']);
        
        $form->text('drop_address', 'Drop Address')->attribute(['id' => 'drop_autocomplete']);
        $form->hidden('drop_lat')->attribute(['id' => 'drop_lat']);
        $form->hidden('drop_lng')->attribute(['id' => 'drop_lng']);
...
document.addEventListener("DOMContentLoaded", function () {
    const MODE = '.intval($geoCodeType).'; // 1 = RouteMappy, else Google
    ...
    function attachAutocomplete(inputId, latId, lngId) {
        ...
        input.addEventListener("input", function () {
            ...
            suggestionBox.innerHTML = "<div style=\'padding:8px\'>Loading...</div>";
            ...
            if (MODE === 1) {
                navigator.geolocation.getCurrentPosition(function (pos) {
                    ...
                    const url = `https://api.routemappy.com/autocomplete?q=${encodeURIComponent(query)}&key='.env('RMP_MAP_KEY').'&lat=${lat}&lon=${lon}`;
                    ...
                    fetch(url)
                        ...
                        .then(data => {
                            ...
                            data.features.forEach(feature => {
                                ...
                                item.textContent = feature.properties.name + ", " + (feature.properties.state || "");
                ...
                // Call Laravel proxy to Google Places API
                fetch(`/api/google-autocomplete?input=${encodeURIComponent(query)}`)
                    .then(res => res.json())
                    .then(data => {
                        suggestionBox.innerHTML = "";
                        data.predictions.forEach(place => {
                            ...
                            item.addEventListener("click", () => {
                                ...
                                fetch(`/api/google-place-details?place_id=${place.place_id}`)
                                    .then(res => res.json())
                                    .then(placeData => {
                                        if (placeData.result && placeData.result.geometry) {
                                            latInput.value = placeData.result.geometry.location.lat;
                                            lngInput.value = placeData.result.geometry.location.lng;
                                            suggestionBox.style.display = "none";
                                        }
                                    });
                            });
        ...
        input.addEventListener("blur", () => {
            setTimeout(() => suggestionBox.style.display = "none", 300);
        });
    }

    attachAutocomplete("pickup_autocomplete", "pickup_lat", "pickup_lng");
    attachAutocomplete("drop_autocomplete", "drop_lat", "drop_lng");
});
</script>
');
        ...
        $form->saved(function (Form $form) {
            ...
            admin_toastr('Booking successfully created!', 'success');
            return redirect('/admin/dispatch_panel');
        });
